...
describe('App', () => {
  ...
  it('captures manual flight keys only after explicit enable and neutralizes on release and Escape', async () => {
    render(<App />)
    fireEvent.click(await screen.findByRole('button', { name: 'Operate' }))
    const enable = await screen.findByRole('button', { name: 'Enable Keyboard Control' })
    ...
    expect(screen.getByRole('button', { name: 'Enable Keyboard Control' })).toBeVisible()
  })
})
